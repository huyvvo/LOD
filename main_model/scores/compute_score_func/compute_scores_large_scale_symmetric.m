function [] = compute_scores_large_scale_symmetric(args, class_indices, row_indices)
...
for cl = class_indices
  ...
  opts.PL = ProposalLoader(opts.prop_path, opts.n, opts.prop_step);
  opts.row_indices = row_indices;
  ...
  for row = row_indices
    time_zero = tic;
    i = full(opts.indices(row, 1)); j = full(opts.indices(row, 2));
    if ~isequal(cache_i, i)
      feati = read_feat(opts, i);
      proposals_i = ProposalLoader_load(opts.PL, i);
      cache_i = i;
    end
    if ~isequal(cache_j, j)
      featj = read_feat(opts, j);
      proposals_j = ProposalLoader_load(opts.PL, j);
      cache_j = j;
    end
...
function [indices] = build_indices(opts)
  if isfield(opts, 'indices_name') && ~isempty(opts.indices_name)
    IL = IndiceLoader(fullfile(opts.nb_root{1}, opts.nb_type{1}, opts.clname, opts.indices_name), '', false);
    indices = IndiceLoader_load_chunk(IL,min(opts.row_indices),max(opts.row_indices));
  else
    n = opts.n; 
...
function [save_path_conf, save_path_std] = get_save_paths(opts)
  save_path_conf = fullfile(opts.root, opts.clname, 'confidence_symmetric', ...
                            sprintf('%s_normalized_%s', opts.scname, opts.num_pos_text));
  save_path_std = fullfile(opts.root, opts.clname, 'standout_symmetric', ...
...
function [] = save_results(opts, left, right, ...
  ...
  if opts.saveconf
    ...
    if isfield(opts, 'indices_name') && ~isempty(opts.indices_name)
      fprintf('Indices.mat is in %s. Indices are not saved in %s\n', ...
              fullfile(opts.nb_root{1}, opts.nb_type{1}, opts.clname, opts.indices_name), ...
              opts.save_path_conf);
    elseif exist(fullfile(opts.save_path_conf, 'indices.mat'), 'file') ~= 2
  ...
  if opts.savestd
    ...
    if isfield(opts, 'indices_name') && ~isempty(opts.indices_name)
      fprintf('Indices.mat is in %s. Indices are not saved in %s\n', ...
              fullfile(opts.nb_root{1}, opts.nb_type{1}, opts.clname, opts.indices_name), ...
              opts.save_path_std);
    elseif exist(fullfile(opts.save_path_std, 'indices.mat'), 'file') ~= 2
      indices = opts.indices;
