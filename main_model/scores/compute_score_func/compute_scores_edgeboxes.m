function [] = compute_scores_edgeboxes(args, class_indices, row_indices)
% [] = compute_scores_edgeboxes(args, class_indices, row_indices)
%
...
for cl = class_indices
  ...
  %-------------------------------
  % load imdb
  [opts.images_size, opts.proposals, eb_scores] = load_imdb(opts);
  ...
  for row = row_indices
    ...
    if ~isequal(cache_i, i)
      feati = read_feat(opts, i) .* eb_scores{i}(:);
      cache_i = i;
    end
    if ~isequal(cache_j, j)
      featj = read_feat(opts, j) .* eb_scores{j}(:);
      cache_j = j;
    end
    [confidence, standout] = compute_scores_(opts, feati, featj, i, j);
    if opts.saveconf
...
function [images_size, proposals, eb_scores] = load_imdb(opts)
  if opts.small_imdb 
    imdb = load(fullfile(opts.root, opts.clname, [opts.clname, '_small_lite.mat']), 'proposals', 'images_size', 'scores');
  else 
    imdb = load(fullfile(opts.root, opts.clname, [opts.clname, '_lite.mat']), 'proposals', 'images_size', 'scores');
  end
  images_size = imdb.images_size;
  proposals = imdb.proposals;
  eb_scores = imdb.scores;
end
...
function [save_path_conf, save_path_std] = get_save_paths(opts)
  save_path_conf = fullfile(opts.root, opts.clname, 'confidence_edgeboxes', ...
                            sprintf('%s_normalized_%s', opts.scname, opts.num_pos_text));
  save_path_std = fullfile(opts.root, opts.clname, 'standout_edgeboxes', ...
...
function [confidence, standout] = compute_scores_(opts, feati, featj, i, j)
  % compute standout
  timer_conf = tic;
  if opts.symmetric
  ...
  timer_std = tic;
  if opts.savestd
    standout = opts.stdfunc(transpose(opts.proposals{i}), transpose(opts.proposals{j}), ...
