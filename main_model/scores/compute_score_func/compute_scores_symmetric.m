...
for cl = class_indices
  ...
  %-------------------------------
  % load imdb
  [opts.images_size, opts.proposals, ~] = load_imdb(opts);
...
function [images_size, proposals, bboxes] = load_imdb(opts)
  if opts.small_imdb 
    imdb = load(fullfile(opts.root, opts.clname, [opts.clname, '_small_lite.mat']), 'proposals', 'images_size', 'bboxes');
  else 
    imdb = load(fullfile(opts.root, opts.clname, [opts.clname, '_lite.mat']), 'proposals', 'images_size', 'bboxes');
  end
