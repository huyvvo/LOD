function [] = compute_scores_large_scale(args, class_indices, row_indices)
% [] = compute_scores_large_scale(args, class_indices, row_indices)
%
...
for cl = class_indices
  ...
  if isempty(opts.indice_path)
    fprintf('Building indices ...\n');
    opts.indices = build_indices(opts);
  else 
    fprintf('Loading indices ...\n');
    opts.indices = getfield(load(sprintf(opts.indice_path, opts.clname)), 'indices');
  end
...
function [save_path_conf, save_path_std] = get_save_paths(opts)
  save_path_conf = fullfile(opts.root, opts.clname, 'confidence', ...
                            sprintf('%s_normalized_%s', opts.scname, opts.num_pos_text));
  save_path_std = fullfile(opts.root, opts.clname, 'standout', ...
