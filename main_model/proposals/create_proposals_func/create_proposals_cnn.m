...
%     -----------------------
% 
%     save_dir: string, name of the folder to which the result will be saved.
%
...
for cl = class_indices
  ...
  for i = row_indices
    ...
    topo_salc = clean_salc(raw_salc, opts.spsize{i}, opts.alpha*max(raw_salc));
    opts.peaks = topo_peaks(topo_salc, opts.num_maxima, opts.ws, opts.alpha*max(raw_salc));
...
function [args] = argument_reader(args)
  assert(all(isfield(args, {...
                'DATA_ROOT', 'root', 'clname', 'small_imdb', ...
                'feat_type', 'save_dir', 'save_root', 'save_result', ...
                'alpha', 'num_maxima', 'ws', 'conn', ...
                'beta_global', 'beta_local', 'num_levels', 'root_threshold', ...
            })));
end
