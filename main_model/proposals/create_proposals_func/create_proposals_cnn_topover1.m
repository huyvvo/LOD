...
for cl = class_indices
  ...
  for i = row_indices
    ...
    opts.peaks = topo_peaks_ver1(topo_salc, opts.num_maxima, opts.ws, opts.alpha*max(raw_salc));
...
function [neighbor_ids] = get_neighbor_ids(idx, h, w)
  ...
  neighbor_ids = sub2ind([h,w],I(valid),J(valid));
end
...
function [local_salc] = build_local_saliency_map(opts, im_idx, peak_idx)
  ...
  for cpid = 1:numel(components.PixelIdxList)
    ...
    if max(local_salc(pixel_ids)) < opts.root_threshold && ...
       max(local_salc(pixel_ids)) < local_salc(peak_idx)
